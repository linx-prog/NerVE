...
output_path = op.join(ROOT_path, 'toronto-3d')
os.makedirs(output_path, exist_ok=True)

device = torch.device('cuda')
toronto3d_config = {
    'data_path': op.join(ROOT_path, 'toronto-3d'),
    'file_list': op.join(ROOT_path, 'toronto-3d', 'data_list.txt'),
    'pc_file': 'pc_obj.ply',
    'grid_size': 64,
    'cube_shift_mode': 'None'
}
toronto3d_dataset = data.get_dataset('RawPCDataset')(toronto3d_config)
...
t0 = time()
with torch.no_grad():
    for idx in range(len(toronto3d_dataset)):
        model_input, info = toronto3d_dataset.get_data(idx, normalize='cube_face')
        ...
        model_input, info = toronto3d_dataset.get_data(idx, normalize='geom')
        model_input = {key: val.cuda() for key,val in model_input.items()}
        model_input['info'] = info
